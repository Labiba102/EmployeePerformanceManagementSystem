...
function ProjManagerAnalytics() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setId(projManagerId);
        const goals = await fetchGoalList(projManagerId);
        setgoalList(goals);
      } catch (error) {
  ...
  // fetches the kpi ratings recieved from the backend by the hr managers' evaluation form 
  ...
  // fetches comments (if any) provided by the hr manager regarding performance =
  ...
  useEffect(() => {
    ...
    // based on the completion date of the goals, it fetches ans fills in data for the line chart to be displayed
  ...
  useEffect(() => {
    // based on the creation date of the goals, and what their status is by the end of month, it fetches ans fills in data for the line chart to be displayed
  ...
  // display of kpis based on the ratings fetched from the backend (displays percentage along with doughnut chart)
  ...
  // renders lowest percentage kpis to be displayed in areas to improve
              ...
              {comments.map((comment, index) => (
                <div className="comment" key={index}>
                  {comment.anonymous && (comment.approved) ? (
                    ...
                    // If the comment is not anonymous, display HR manager's name and comment
                    <div>
                      <div>
                        <span className="who-commented">HR Manager</span>
                      </div>
                      <div className="comment-description">
                        {comment.comment}
                      </div>
...
export default ProjManagerAnalytics;
